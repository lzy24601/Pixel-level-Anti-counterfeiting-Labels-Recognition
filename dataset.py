...
def get_dataloader(
    ...
    # Image Folder
        ...
        transform = transforms.Compose([
            ...
            #  根据概率对图片进行水平（左右）翻转，每次根据概率来决定是否执行翻转 默认概率0.5
             transforms.RandomHorizontalFlip(),
             transforms.ToTensor(),
             transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
             ])
        if withclass:
            train_set = ImageFolderWithClass(root_dir, transform)
        else:
...
class ImageFolderWithClass(ImageFolder):
    def __init__(
            self,
            root,
            transform= None,
    ):
        super(ImageFolderWithClass, self).__init__(root, transform)
    ...
    def __getitem__(self, index: int):
        ...
        class_label = int(os.path.basename(path).split('.')[0].split('_')[0])
